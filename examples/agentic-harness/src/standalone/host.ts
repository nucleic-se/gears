...
export class StandaloneHarness {
    ...
    static async open(options: HarnessOptions): Promise<StandaloneHarness> {
        ...
        return createHarness().compose({
            extensions: [
                { id: 'runtime.gears', version: '7', apiVersion: 1, configuration: JSON.stringify({ modelTimeoutMs, outputTokens: options.outputTokens ?? 1800, tools: (options.tools ?? []).map(tool => ({ definition: tool.definition, effect: tool.effect })) }), roles: { runtime: () => StandaloneHarness.openRuntime(options) } },
                { id: 'provider.gears', version: '1', apiVersion: 1, roles: { provider: () => options.provider } },
                { id: 'context.gears', version: '4', apiVersion: 1, configuration: JSON.stringify({ tokens: options.contextTokens ?? 16000, custom: options.context ? options.composition : undefined }), roles: { context: () => options.context ?? budgetedContext('', options.contextTokens ?? 16000, {
                    maxToolResultCharacters: 4000,
                    minRecentGroups: 3, // Two recent exchanges plus the transient state message.
                    referenceToolResult: (message, index, tools) => message.toolName !== 'read_tool_result' && tools.some(tool => tool.name === 'read_tool_result')
