    ...
    static async open(options: HarnessOptions): Promise<StandaloneHarness> {
        ...
        return createHarness().compose({
            extensions: [
                ...
                { id: 'context.gears', version: '12', apiVersion: 1, configuration: JSON.stringify({ tokens: options.contextTokens ?? 16000, custom: options.context ? options.composition : undefined }), roles: { context: () => options.context ?? budgetedContext('', options.contextTokens ?? 16000, {
