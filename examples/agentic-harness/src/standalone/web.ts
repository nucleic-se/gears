    ...
    const server = createServer(async (req, res) => {
        ...
        try {
            ...
            if (req.method === 'GET' && url.pathname === '/api/tasks')
                ...
                const id = parts[2];
                if (req.method === 'GET' && parts[3] === 'inspect' && parts.length === 4)
                    result = await host.inspect(id, count(url.searchParams.get('after'), 0, Number.MAX_SAFE_INTEGER));
                else if (req.method === 'GET' && parts[3] === 'events')
...
async function refresh(){if(busy||!token)return;busy=true;try{const trees=await api('tasks?offset='+offset);const snapshot=JSON.stringify(trees);if(snapshot===lastRender)return;lastRender=snapshot;$('tasks').replaceChildren();for(const tree of trees){const card=el('section','',$('tasks'),'card');el('small',tree.id+' · '+tree.modelCalls+'/'+tree.limits.modelCalls+' model calls · '+(tree.usage.inputTokens+tree.usage.outputTokens)+' tokens',card);for(const t of tree.tasks){const box=el('div','',card,t.parentId?'child':'');el('h3',t.title,box);el('div',t.phase+(t.wakeAt?' · wakes '+new Date(t.wakeAt).toLocaleString():''),box,'status');if(t.notes)el('pre',t.notes,box);if(t.answer)el('pre',t.answer,box);if(t.error)el('pre',t.error,box);button('Message',box,async()=>{const message=prompt('Message for this task');if(message)await api('tasks/'+tree.id+'/message',{taskId:t.id,message});await refresh()});button('Stop',box,async()=>{await api('tasks/'+tree.id+'/cancel',{taskId:t.id});await refresh()})}button('Inspect machinery',card,()=>inspectTree(tree.id));for(const name of tree.artifacts)button('Artifact: '+name,card,async()=>{const r=await api('tasks/'+tree.id+'/artifact?name='+encodeURIComponent(name));const viewer=$('artifact');viewer.hidden=false;viewer.replaceChildren();el('h3',name,viewer);const view=el('pre',r.content??'Unavailable',viewer);view.scrollIntoView()})} $('error').textContent=''}catch(e){$('error').textContent=e.message}finally{busy=false}}
async function inspectTree(id,after=0){
const snapshot=await api('tasks/'+id+'/inspect?after='+after),panel=$('inspector');panel.hidden=false;panel.replaceChildren();
el('h2','Harness inspector',panel);el('p','Captured '+new Date(snapshot.capturedAt).toLocaleString()+' · revision '+snapshot.revision, panel);
el('p','Committed state and exact provider-adapter requests. Refresh to capture again.',panel);
button('Refresh snapshot',panel,()=>inspectTree(id));
if(snapshot.events.length&&snapshot.nextSequence<snapshot.revision)button('Next trace page',panel,()=>inspectTree(id,snapshot.nextSequence));
button('Download JSON',panel,()=>{const url=URL.createObjectURL(new Blob([JSON.stringify(snapshot,null,2)],{type:'application/json'}));const a=document.createElement('a');a.href=url;a.download='gears-inspection.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000)});
button('Close',panel,()=>{panel.hidden=true});
function detail(label,value){const d=el('details','',panel);el('summary',label,d);el('pre',JSON.stringify(value,null,2),d)}
detail('Task tree, messages, progress and budgets',snapshot.state);
for(const event of snapshot.events)detail('#'+event.sequence+' · '+event.type+' · '+(event.taskId||''),event);
