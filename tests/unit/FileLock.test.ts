...
import { FileLock, StaleFileLockError } from '../../src/core/utils/FileLock.js';
...
describe('FileLock', () => {
    ...
    it('never replaces a stale lock by pathname', async () => {
        await fs.writeFile(lockPath, '2147483647');
        const lock = new FileLock(targetPath);

        await expect(lock.acquire(1, 0)).rejects.toBeInstanceOf(StaleFileLockError);
        await expect(fs.readFile(lockPath, 'utf8')).resolves.toBe('2147483647');
    });
});
